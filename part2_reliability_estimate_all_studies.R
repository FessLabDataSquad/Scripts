...
task1=study1_ratings[,c(1,3:7)]
task2=study2_ratings[,c(1,3:7)]
...
task1$mean=rowMeans(task1[,2:6], na.rm=T)
task2$mean=rowMeans(task2[,2:6], na.rm=T)
...
cor(task1[2:7], use="pairwise.complete.obs")
cor(task2[2:7], use="pairwise.complete.obs")
...
friendly1=study1_ratings[,c(1,8:12)]
friendly2=study2_ratings[,c(1,8:12)]
...
friendly1$mean=rowMeans(friendly1[,2:6], na.rm=T)
friendly2$mean=rowMeans(friendly2[,2:6], na.rm=T)
friendly3$mean=rowMeans(friendly3[,2:5], na.rm=T)
friendly4$mean=rowMeans(friendly4[,2:5], na.rm=T)

#Inspect correlations between (1) each rater and the group's average, and (2) each pairwise correlation.  
cor(friendly1[2:7], use="pairwise.complete.obs")
cor(friendly2[2:7], use="pairwise.complete.obs")
